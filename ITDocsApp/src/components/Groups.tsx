...
interface GroupFormProps {
  initial?: Group
  onSave: (d: Omit<Group, 'id' | 'createdAt'>) => void
  onClose: () => void
}
        ...
        <div className="px-5 py-4 space-y-3.5 max-h-[65vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="relative">
              <select value={form.type} onChange={e => set('type', e.target.value as GroupType)} className={inp() + ' appearance-none pr-8'}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {form.members.map(m => (
                <span key={m} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-navy-700 border border-edge-subtle text-[11px] text-ink-secondary font-mono">
                  {m}
                  <button type="button" onClick={() => set('members', form.members.filter(x => x !== m))} className="text-ink-muted hover:text-red-400 transition-colors ml-0.5"><X size={9} /></button>
                </span>
              ))}
            </div>
            <div className="flex gap-2">
              <input value={memberInput} onChange={e => setMemberInput(e.target.value)}
                ...
                placeholder="Type name/username, press Enter…" className={inp() + ' flex-1'} />
              <button type="button" onClick={addMember} className="px-3 py-2 rounded-lg bg-navy-700 border border-edge-default text-ink-secondary text-xs hover:bg-navy-600 transition-colors">Add</button>
            </div>
          </div>
              ...
              <div className="max-h-32 overflow-y-auto rounded-lg bg-navy-700 border border-edge-default divide-y divide-edge-subtle">
                {assets.map(a => (
                  <label key={a.id} className="flex items-center gap-2.5 px-3 py-2 cursor-pointer hover:bg-navy-600 transition-colors">
                    <div onClick={() => toggleAsset(a.id)} className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 transition-all cursor-pointer ${form.linkedAssets.includes(a.id) ? 'bg-blue-500 border-blue-500' : 'border-edge-strong'}`}>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {form.tags.map(t => (
                <span key={t} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-navy-700 border border-edge-subtle text-[11px] text-ink-secondary font-mono">
                  {t}
                  <button type="button" onClick={() => set('tags', form.tags.filter(x => x !== t))} className="text-ink-muted hover:text-red-400 transition-colors ml-0.5"><X size={9} /></button>
                </span>
              ))}
            </div>
            <div className="flex gap-2">
...
function GroupDetail({ group, onBack, onEdit, onDelete }: {
  ...
  const addMember = () => {
    ...
    if (m && !group.members.includes(m)) {
      updateGroup({ ...group, members: [...group.members, m] })
    }
    setMemberInput('')
  }

  const removeMember = (m: string) => {
    updateGroup({ ...group, members: group.members.filter(x => x !== m) })
  }
...
export default function Groups() {
  const { groups, addGroup, updateGroup, deleteGroup, toast } = useApp()
  ...
  const handleDelete = () => {
    ...
    setSelectedId(filtered.find(g => g.id !== selected.id)?.id ?? '')
  }
