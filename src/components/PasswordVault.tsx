...
import {
  ...
  Edit2, Star, X, Trash2, RefreshCw, Shield, ChevronDown,
} from 'lucide-react'
...
export default function PasswordVault() {
  const { passwords, addPassword, updatePassword, deletePassword, toggleStarPassword } = useApp()
    ...
    <div className="flex h-full">
      ...
      <div className="w-72 border-r border-edge-subtle bg-navy-900 flex flex-col flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-edge-subtle">
          ...
          {filtered.map(p => (
            <button key={p.id} onClick={() => { setSelectedId(p.id); setRevealed(false) }}
              className={`w-full px-4 py-3 text-left transition-all hover:bg-navy-800/80 group ${selectedId === p.id ? 'bg-navy-800 border-r-2 border-blue-500' : ''}`}>
          ...
          <div className="p-6 max-w-2xl">
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-navy-700 border border-edge-default flex items-center justify-center flex-shrink-0">
                  <KeyRound size={18} className="text-blue-400" />
                </div>
                <div>
                  <h1 className="text-lg font-semibold text-ink-primary">{selected.name}</h1>
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={() => toggleStarPassword(selected.id)}
                  ...
                  <Star size={14} fill={selected.starred ? 'currentColor' : 'none'} />
                </button>
                <button onClick={() => setEditEntry(selected)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-navy-800 border border-edge-default text-ink-secondary text-xs hover:border-edge-strong transition-colors">
                  <Edit2 size={12} /> Edit
                </button>
                <button onClick={() => setDeleteTarget(selected)} className="p-2 rounded-lg bg-navy-800 border border-edge-default text-ink-muted hover:text-red-400 hover:border-red-500/30 transition-all">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
            ...
            {/* Credentials card */}
            <div className="bg-navy-800 border border-edge-subtle rounded-xl p-5 mb-4 space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <div className="flex-1 px-3 py-2.5 rounded-lg bg-navy-700 border border-edge-default font-mono text-sm text-ink-primary select-all">{selected.username}</div>
                  <button onClick={() => copy(selected.username, 'user')}
                    className={`p-2.5 rounded-lg border transition-all ${copied === 'user' ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-navy-700 border-edge-default text-ink-muted hover:text-blue-400 hover:border-blue-500/30'}`}>
                    <Copy size={13} />
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <div className="flex-1 px-3 py-2.5 rounded-lg bg-navy-700 border border-edge-default font-mono text-sm text-ink-primary tracking-widest select-all">
                    {revealed ? selected.password : '•'.repeat(Math.min(selected.password.length, 20))}
                  </div>
                  <button onClick={() => setRevealed(!revealed)}
                    className={`p-2.5 rounded-lg border transition-all ${revealed ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' : 'bg-navy-700 border-edge-default text-ink-muted hover:text-ink-primary'}`}>
                    {revealed ? <EyeOff size={13} /> : <Eye size={13} />}
                  </button>
                  <button onClick={() => copy(selected.password, 'pass')}
                    className={`p-2.5 rounded-lg border transition-all ${copied === 'pass' ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-navy-700 border-edge-default text-ink-muted hover:text-blue-400 hover:border-blue-500/30'}`}>
                    <Copy size={13} />
                  </button>
                </div>
            ...
            {/* Notes */}
            {selected.notes && (
              <div className="bg-navy-800 border border-edge-subtle rounded-xl p-5 mb-4">
